...
def inbox(request):
    profile = request.user.profile
    messageAll = profile.messages.all()
    unreadCount = messageAll.filter(is_read=False).count()
    context = {
...
def inboxMessage(request, pk):
    ...
    messageInbox = profile.messages.get(id=pk)

    if messageInbox.is_read == False:
        messageInbox.is_read = True
        messageInbox.save()
...
def createMessage(request, pk):
    ...
    try:
        sender = request.request.profile
    except:
        sender = None
